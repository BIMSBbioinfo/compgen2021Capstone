...
Hands-on Data Analysis 2021: Machine Learning for Genomics. You can find more information about the course
on the course [page.](https://compgen.mdc-berlin.de/) You can access the the Github
...
* Cancer cells have altered gene expression. There are
many genes that are abberantly turned on or off (gene activation or gene silencing) that
dramatically alter the overall activity of the cell. In cancer, a gene that is not normally
expressed in that cell or expressed in high levels can be switched on and expressed at high levels.
...
* `CNV` stands for **Copy Number of Variation**. A `CNV` occurs is when the number of
...
For PDX dataset as well, AUC values are derived from tumor volume change. 
...
* **HINT :** For the ensemble model, you can simply average output or re-train an ensembl model with cross-validation. 
Anything is OK but not every approach will be performant. 
