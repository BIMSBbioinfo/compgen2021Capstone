...
After removal of the effect, train drug response prediction on cell line dataset(s) and predict on PDX. Report if R-squared improved over TASK1  or not.
You can try multiple batch removal methods. If that's the case, report PDX performance for each of the methods. 
...
of the cell line datasets but PDX dataset must be in there prior to batch effect removal.
After removal of the effect, train drug response prediction on cell line dataset(s) and predict on PDX. Report if R-squared improved over TASK 2 or not.
You can try multiple batch removal methods. If that's the case, report PDX performance for each of the methods. 
...
- use SVD, find eigenvectors associated with batch class, remove and reconstruct them.
...
One easy trick to try on binary matrices is to set 0s to -1. Another one could be to try logistic regression residuals instead of linear regression residuals as
`regressBatches` function does. You should probably try to use these methods in each omic data type seperately and construct corrected matrices for each of them.
