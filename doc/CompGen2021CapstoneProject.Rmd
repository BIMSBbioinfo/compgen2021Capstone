...
In addition, you will need to fill a form on the results to be able
...
* **NOTE : ** To use mutation data, you need to make it binary. However, you may
...
sample taken by biopsy from the patient by xenografting into the mice and letting it grow there. We use the
`PDX` data to obtain more meaningful results clinically. In Figure below you
...
data before machine learning, one way to do this is provided below in the data summary sections. Other similar methods also exists here:  https://datatricks.co.uk/one-hot-encoding-in-r-three-simple-methods
...
single omic datasets into a multi-omics matrix prior to training. You can also experiment to see how different data processing techniques effects 
...
make a table or heatmap (with values on the cells) for the CV R-squared values. 
columns: GDSC_Sanger, GDSC_Broad, CCLE and rows: ALPELISIB, BUPARLISIB, CETUXIMAB, ERLOTINIB, RIBOCICLIB, TRAMETINIB   
...
* **IMPORTANT NOTE :** `the PDX data set` is for `validation`  **ONLY** and it
...
This task is about using panel sequencing data to predict drug response (CNVs and mutations for couple hundred to a thousand genes). The datasets to be used 
are in `oncoKB_intersect_genes` and `drug_response` folders in `GDSC_Broad`, `GDSC_Sanger`, `CCLE` and `PDX_all` folders. In many clinical
`PDX_all` should not be used for training, only for testing. The model parameters during training should not be optimized
...
datasets. More complicated methods for batch effect correction is on Task 3 and don't belong in this task. 
Just check if normalization, transformation and filtering is needed. And apply whatever you think is
necessary. You can also experiment to see how different data processing techniques effects 
accuracy. But those experiments do not belong in the final report, just include the final
processing method and state if you did some experimentation to arrive at that method.
...
4. Compare the best performing models for PDX prediction from TASK 1 and TASK2. Make informative figures and briefly discuss (2-3 sentences) if multi-omics or panel sequencing is better for modeling response judging from PDX results. 
