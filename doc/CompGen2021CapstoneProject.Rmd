...
author: Altuna Akalin,Vedran Franke,Bora Uyar,Jan Dohmen,Artem Baranovsky,İrem Begüm Gündüz
Date: "`r Sys.Date()`"
output:
  html_document:
    toc: yes
...
response variable that we will a training model to predict. There is six
...
* `Genomics of Drug Sensitivity in Cancer (GDSC)` consists of two batches:
  ...
  2. SANGER half of cell lines sequences in Sanger from the SANGER cell line library which you can find in `GDSC_SANGER` data folder.

**NOTE : **  Even though they might have overlapped cell lines,`GDSC_BROAD`
...
actual tumors despite using so widely in researches. The reason for this is the
...
mouse and divided into slices ,and cultured. Tumors are solid structures that
consist of cells that are completely focused on division. The difference between
`CMA` and tumor structures because only from the growth and division of cells
located on the outside of the plate. The most important reason for this is that
the cells in the internal structure of the tumor have limited access to oxygen.
As a result, `CMA` is extremely useful in cancer research, but does not fully
represent tumor cell structure.
...
sample is taken by biopsy from the patient by injecting it into mice. We use the
...
knitr::include_graphics(c("https://www.researchgate.net/publication/292953008/figure/fig1/AS:565678451027968@1511879660297/Generation-of-PDX-models-Surgical-specimens-from-cancer-patients-are-divided-into-small.png","https://kosheeka.com/wp-content/uploads/2020/07/tumor-primary-cultures.jpg"))
```

 
***

* The `Gene expression` measurement is usually achieved by quantifying levels of
the gene product, which is often a protein. Two common techniques used for
protein quantification include Western blotting and enzyme-linked immunosorbent
assay or ELISA.Cancer can be described as a disease of altered gene expression.
There are many proteins that are turned on or off (gene activation or gene
silencing) that dramatically alter the overall activity of the cell. A gene that
is not normally expressed in that cell can be switched on and expressed at high
levels.
...
model to predict drug response when you train a a model with one data set,and
validate to another? Do you have a good validation? accuracy on the other one?
* **HINT:** You have large numbers of feature data and, you can use which
one you'd like. But, not all of the features are informative considering the the
...
your model. You will need to jointly normalize the datasets together to be able to use them in training and validation. You may also need to remove non-informative
...
A histogram and a bar plot were plotted for representing the frequency of the
gene expression of `the GDSC BROAD` dataset. There are no missing values within
the dataset. Check the distribution of data. The dimensions of the gene
